...
int addFile(File* newFile){
    ...
    pthread_mutex_lock(&((*storageList)->mutex));

    if( (*storageList) == NULL){
        (*storageList) = newFile;
        storageTail = (*storageList);
       return  pthread_mutex_unlock(&((*storageList)->mutex));;
   ...
   return  pthread_mutex_unlock(&(storageTail->mutex));
}
...
File* findFileAndPrec(char* nameF, File** prec){
    ...
    if(strncmp((*storageList)->name, nameF, (ssize_t)strlen(nameF)+1) == 0){
        ...
        (*prec) = NULL;
        if( pthread_mutex_lock(&((*storageList)->mutex)) == 0)
                return (*storageList);
        else return NULL;
    }
...
File* FindVictims(int n, int fd, int* victimsCount){
    ...
    if((*storageList) == NULL){
        pthread_mutex_unlock(&(*storageList)->mutex);
        return NULL;
    }
